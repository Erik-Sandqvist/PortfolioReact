...
export const Footer = () => {
  const debugId = useMemo(() => `footer-${Date.now()}`, []);

  useEffect(() => {
    try {
      console.log('DEBUG: Footer mounted', debugId);
      // print limited stack to help find render origin
      const stack = new Error().stack?.split('\n').slice(1,6).join('\n');
      console.log('DEBUG: Footer stack:\n', stack);
    } catch (e) {
      console.warn('DEBUG: Footer log failed', e);
    }
  }, [debugId]);

  return (
    <footer data-debug-id={debugId} className="relative z-20 flex justify-between items-center font-onest px-5 sm:px-6 md:px-12 lg:px-72 py-2 w-full bg-[rgba(8, 26, 58, 0.8)] backdrop-blur-lg border-t border-yellow/10 shadow-lg">
      {/* Vänster sektion */}
      <div>
