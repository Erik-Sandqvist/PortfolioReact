...
const Torst = () => {
  ...
  const images = [
    {
      src: asset("Screenshot 2026-06-18 214440.png"),
      alt: "Törst screenshot 1",
    },
    {
          ...
          <div className="grid gap-4 md:grid-cols-[1.2fr_0.8fr]">
            ...
            <div className="space-y-4">
              <div className="flex items-end justify-between gap-4">
                <div>
                  <h3 className="text-lg font-semibold text-primary">Gallery</h3>
                  <p className="text-sm text-secondary/80">Key pages and sections from the live site.</p>
                </div>
              </div>

              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 auto-rows-[280px] md:auto-rows-[320px]">
                {images.map((image, idx) => (
                  <figure
                    key={image.src}
                    className="overflow-hidden rounded-xl border border-secondary/20 bg-black/10 shadow-lg transition-all duration-300 hover:shadow-xl hover:border-secondary/40"
                  >
                    <img
                      src={image.src}
                      alt={image.alt}
